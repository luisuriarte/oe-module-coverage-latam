...
if (in_array($action, ['create', 'update', 'toggle', 'delete'], true)) {
    ...
    if (!AclMain::aclCheckCore('admin', 'docs')) {
        covl_json(['error' => 'Sin permisos para modificar reglas'], 403);
    }
}
...
try {
    switch ($action) {
        ...
        case 'get':
            ...
            if ($row === null) {
                covl_json(['error' => "Regla {$id} no encontrada"], 404);
            }
            covl_json($row);
        ...
        case 'create':
            ...
            if (empty($data['code_type']) || empty($data['auth_mode']) || empty($data['insurance_company_id'])) {
                covl_json(['error' => 'Campos requeridos: insurance_company_id, code_type, auth_mode'], 422);
            }
        ...
        // -----------------------------------------------------------------------
        default:
            covl_json(['error' => "Acción desconocida: {$action}"], 400);
    }
} catch (\Throwable $e) {
    error_log('[covl] api/auth_rules.php error: ' . $e->getMessage());
    covl_json(['error' => 'Error interno del servidor'], 500);
}
